...
public class UI_QuanLy2 extends JFrame implements ActionListener{
	private JButton btnQuanLy, btnDangXuat, btnThoat;
	...
	private JTextField txtMaNV, txtHo, txtTen, txtTuoi, txtTienluong, txtNhapMa;
	...
	private Box quanLyLinhKien() {
		...
		b1.add(lblLoai = new JLabel("     Mã loại: "));
		b1.add(cboLoai = new JComboBox<String>());
		cboLoai.setEditable(false);
		cboLoai.setMaximumSize(new Dimension(200, 20));
		...
		String[] str = { "Mã linh kiện", "Tên linh kiện", "Loại", "Số lượng", "Đơn giá nhập", "Đơn giá bán",
				"Ngày nhập" };
		...
		for (LinhKien lk : dslk) {
			tbModelLK.addRow(new Object[] { lk.getMaLinhkien(), lk.getTenLinhkien(),
					loai_dao.getLoaiTheoMaL(lk.getLoai().getMaLoai()).getTenLoai(), lk.getSoLuong(),
	...
	private Box quanLyHoaDon() {
		...
		for (HoaDon hoaDon : dsHoaDon) {
			tbModelHD.addRow(new Object[] { hoaDon.getMaHD(), hoaDon.getMaNhanVien(), hoaDon.getMaKhach(),
					hoaDon.getNgayBan(), formatter.format(hoaDon.getTongTien() * 1000) });
		}
		JPanel bottom = new JPanel();
		bottom.setBorder(BorderFactory.createTitledBorder("Tác vụ"));
		...
		tableHD.getSelectionModel().addListSelectionListener(new ListSelectionListener(){
			...
			public void valueChanged(ListSelectionEvent e) {
				if (tableHD.getSelectedRowCount() == 1) {
					...
					txtNgayBan.setText(hoaDon.getNgayBan().toString());
				}	
			}
	    });
	...
	public boolean KiemTraKH() {
		...
		/*
		 * else if (!txtMaKH.getText().trim().matches("[KH][0-9]{5}")) {
		 * JOptionPane.showMessageDialog(this, "Mã khách hàng có định dạng: KH99999");
		 * txtMaKH.requestFocus(); txtMaKH.selectAll(); return false; }
		 */		return true;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// TODO Auto-generated method stub
				...
				//////////
					...
					String maloaiLK = cboLoai.getSelectedItem().toString();
					Loai loaiLK = new Loai_DAO().getLoaiTheoMaL(maloaiLK);
					 lk = new LinhKien(txtMaLK.getText().trim(), txtTenLK.getText().trim(), loaiLK,
					...
					if (KiemTraKH()) {
						...
						if(kh != null) {
							JOptionPane.showMessageDialog(this, "Mã khach hàng đã tồn tại");
							return;
						}
					...
					if (loai_dao.create(loai)) {
						...
						cboLoai.addItem(txtMaLoai.getText());
						JOptionPane.showMessageDialog(this, "Lưu thành công");
					} else
						JOptionPane.showMessageDialog(this, "Lưu không thành công");
				} 
					...
					if (tableHD.getSelectedRow() == -1)
						...
						if (hd_dao.delete(String.valueOf(tbModelHD.getValueAt(row, 0)))) {
					...
					if (tableLoai.getSelectedRow() == -1)
						...
						if (KiemTraLoai()) {
							...
							if (loai_dao.update(loai)) {
								tbModelLoai.setValueAt(txtMaLoai.getText(), row, 1);
								tbModelLoai.setValueAt(txtTenLoai.getText(), row, 2);
								JOptionPane.showMessageDialog(this, "Sửa thông tin thành công");
							} else
								JOptionPane.showMessageDialog(this, "Sửa thông tin không thành công");
						}
					...
					if (tableHD.getSelectedRow() == -1)
						...
						if (KiemTraHD()) {
							int row = tableHD.getSelectedRow();
							HoaDon hoaDon = new HoaDon(txtMaHD.getText(), txtMaNV2.getText(), txtMaKH2.getText(), Date.valueOf(txtNgayBan.getText()),Double.valueOf(txtTongTien.getText()) );
							if (hd_dao.update(hoaDon)) {
								tbModelHD.setValueAt(hoaDon.getNgayBan().toString(), row, 3);
								tbModelHD.setValueAt(formatter.format(hoaDon.getTongTien() * 1000), row, 4);
					...
					if (row == -1)
						...
						if (KiemTraKH()) {
							...
							if (kh_dao.update(kh)) {
								...
								tbModelKH.setValueAt(kh.getDiaChi() ,row, 3);
					...
					if (row == -1)
						...
						if (KiemTraLK()) {
							...
							LinhKien lk = new LinhKien(txtMaLK.getText().trim(), txtTenLK.getText(),loai ,Integer.parseInt(txtSoLuong.getText()), 
							...
							if (lk_dao.update(lk)) {
								...
								tbModelLK.setValueAt(lk.getSoLuong() ,row, 3);
								tbModelLK.setValueAt(formatter.format(lk.getDonGiaNhap() * 1000), row, 4);
								tbModelLK.setValueAt(formatter.format(lk.getDonGiaBan() * 1000), row, 5);
								tbModelLK.setValueAt(lk.getNgayNhap().toString(), row, 6);
								JOptionPane.showMessageDialog(this, "Sửa thông tin thành công");
							} else
								JOptionPane.showMessageDialog(this, "Sửa thông tin không thành công");
						}
					...
					if(lk == null) {
						...
						tbModelLK.addRow(new Object[] {lk.getMaLinhkien(),lk.getTenLinhkien(),loai_dao.getLoaiTheoMaL(lk.getLoai().getMaLoai()).getTenLoai(),lk.getSoLuong()
								,String.valueOf(formatter.format(lk.getDonGiaNhap() * 1000)),String.valueOf(formatter.format(lk.getDonGiaBan() * 1000)),lk.getNgayNhap()});
			}
					...
					if(hd == null) {
						...
						tbModelHD.addRow(new Object[] {hd.getMaHD(),hd.getMaKhach(),hd.getMaNhanVien(),hd.getNgayBan(),String.valueOf(formatter.format(hd.getTongTien()*1000))});
				}
			}


			private void updateCombobox() {
				Loai_DAO dsLoai = new Loai_DAO();
				ArrayList<Loai> dsLoais = dsLoai.getAllLoai();
				for (Loai loai : dsLoais) {
